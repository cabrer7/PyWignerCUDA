   ...
   pycuda::complex<double> *W41, pycuda::complex<double> *W42, pycuda::complex<double> *W43, pycuda::complex<double> *W44 )
{
    ...
    double lambdap1 =  lambda1/2. ;

    double shell = sqrt( pow( mass_half*c , 2 ) + pow(lambdap1 , 2)  );

    pycuda::complex<double> U11 = pycuda::complex<double>( cos( dt*c*shell ), 0.); //- mass_half*c*sin(c*dt*shell)/shell );
    pycuda::complex<double> U44 = pycuda::complex<double>( cos( dt*c*shell ), 0.); //  mass_half*c*sin(c*dt*shell)/shell );
...
DiracKvNPropagator_XTheta_source_Base = """
//
//   source code for the right Dirac KvN propagator without vector potential interaction 
//   
...
__device__  double D_1_Potential0(double t, double x)
{
    return %s ;
}
...
__global__ void KvN_XTheta_Kernel(
   pycuda::complex<double> *W11, pycuda::complex<double> *W12, pycuda::complex<double> *W13, pycuda::complex<double> *W14,
   pycuda::complex<double> *W21, pycuda::complex<double> *W22, pycuda::complex<double> *W23, pycuda::complex<double> *W24,
   pycuda::complex<double> *W31, pycuda::complex<double> *W32, pycuda::complex<double> *W33, pycuda::complex<double> *W34,
   pycuda::complex<double> *W41, pycuda::complex<double> *W42, pycuda::complex<double> *W43, pycuda::complex<double> *W44, 
   double t )
{
  ...
  pycuda::complex<double>  expDA0 = exp( -dt*D_Theta* theta*theta + I*dt*D_1_Potential0(t,x)*theta    );	
  
  W11[indexTotal]  *=  expDA0;
  W12[indexTotal]  *=  expDA0;
  W13[indexTotal]  *=  expDA0;
  W14[indexTotal]  *=  expDA0;

  W21[indexTotal]  *=  expDA0;
  W22[indexTotal]  *=  expDA0;
  W23[indexTotal]  *=  expDA0;
  W24[indexTotal]  *=  expDA0;

  W31[indexTotal]  *=  expDA0;
  W32[indexTotal]  *=  expDA0;
  W33[indexTotal]  *=  expDA0;
  W34[indexTotal]  *=  expDA0;

  W41[indexTotal]  *=  expDA0;
  W42[indexTotal]  *=  expDA0;
  W43[indexTotal]  *=  expDA0;
  W44[indexTotal]  *=  expDA0;
                ...
                int sign )
{
    ...
    double Lambda     = 0.                             ; //  dlambda*( j - 0.5*X_gridDIM  );
...
class GPU_WignerDiracKvN2D_4x4:
	"""
	...
	def __init__(self, X_gridDIM, P_gridDIM, X_amplitude, P_amplitude,
		...
		self.CUDA_constants +=  '__constant__ double mass_half=%f;'%( 0.00001 )
...
#-------------------------------------------------------------------------------------------------------------------
	...
	def Compiling_CUDA_functions(self):
		...
		#
		DiracKvNPropagator_Theta_source = DiracKvNPropagator_XTheta_source_Base%(
				self.CUDA_constants,self.D_1_Potential_0_String)		
		self.DiracKvNPropagator_Theta  =  \
			SourceModule( DiracKvNPropagator_Theta_source ).get_function( "KvN_XTheta_Kernel" )
